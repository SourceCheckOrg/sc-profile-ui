...
export default function Profile() {
  ...
  async function deployProfile() {
    const profileUrl = "https://profile.sourcecheck.org/";
    let profileAddr;
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8 mt-5 md:mt-0 md:col-span-2">
              ...
              <div className="shadow sm:rounded-md sm:overflow-hidden mt-6">
                <div className="px-4 py-5 bg-white space-y-5 sm:p-6">
                  ...
                  <div className={`${user.eth_profile_addr ? 'hidden' : 'block'} bg-white text-right`}>
                    { deploying ? (
                      <LoaderButton color="indigo" loading={true} />
                    ) : (
                      <Button label="Deploy Profile" color="indigo" onClick={deployProfile} disabled={!account || !(domain || twitter) } />
                    )}
                  </div>
                </div> 
              </div>
